...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['send'])) {
    ...
    $apiKey = $credentials['wa_apikey'] ?? '';
    $numberKey = $credentials['wa_numberkey'] ?? '';

    // Pengecekan API key dan number key
    if (empty($apiKey) || empty($numberKey)) {
        $_SESSION['toast_message'] = 'Api Key atau Number Key tidak ditemukan atau kosong!';
    ...
    $reminderContent = trim($_POST['reminderContent']);
    // $reminderMessages = explode('<br>', $reminderContent);
    $reminderMessages = json_decode($reminderContent, true);


    // Check if message is empty
    if (empty($message) && empty($reminderContent)) {
    ...
    switch ($sendMethod) {
        case 'SEND_SISWA':
            ...
            if (empty($siswaIds)) {
                ...
                foreach ($siswaIds as $siswaId) {
                    if (isset($reminderMessages[$siswaId])) {
                        foreach ($reminderMessages[$siswaId] as $messageContent) {
                            $requestData['target'] = $siswaId;
                            $requestData['description'] = strip_tags($messageContent);
        ...
        case 'SEND_KELAS':
            if (isset($_POST['kelas'])) {
                ...
                if (!empty($reminderMessages)) {
                    // Send reminders to each student in the class
                    foreach ($reminderMessages as $id_siswa => $messages) {
                        foreach ($messages as $messageContent) {
                            $requestData = [
                                'project_name' => $userData['project_name'],
                                'method' => 'SEND_SISWA',
                                'target' => $id_siswa, // Assuming this is the student ID
                                'description' => strip_tags($messageContent['message']),
                            ];
...
<body class="bg-gray-100">
    <div class="flex">
                ...
                <ul class="ml-2 text-gray-300">
                    ...
                    <li class="relative group px-3 sm:ml-12 py-2 mt-4 mb-4 hover:bg-white hover:text-black <?php echo ($current_page == 'tagihan.php') ? 'bg-white underline underline-offset-8 rounded-l text-sky-800' : ''; ?>">
                        <a href="tagihan.php" class="flex items-center transition-all">
                            <svg class="w-5 h-5 mr-2 -rotate-45" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-6">
        ...
        <div id="main-content" class="flex-1 transition-all duration-500 ml-14 sm:ml-60">
            ...
            <header id="header-bar" class="flex justify-between items-center mb-5 min-h-14 bg-slate-50 shadow-sm shadow-slate-400 fixed top-14 left-0 right-0 z-20 sm:ml-60">
                <h1 id="header-title" class="ml-20 sm:ml-10 text-lg font-semibold">Reminder - WA</h1>
            </header>
            ...
            <main class="container mx-auto px-4 py-4 mt-28">
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                    ...
                    <div class="bg-white shadow-md rounded-lg p-6">
                        <h2 class="text-xl font-bold mb-4">Reminder - WA</h2>
                        <form action="" method="post" id="sendMessageForm">
                            ...
                            <div class="mb-4" id="sekolahFieldSiswa">
                            ...
                            <!-- Dropdown Pilih Sekolah -->
                            <div class="mb-4 hidden" id="sekolahField">
                                <label for="sekolah" class="block text-gray-700 font-medium mb-2">Pilih Sekolah :</label>
                            ...
                            <!-- Reminderbox -->
                            <div id="reminderBoxField" class="mb-4">
                                <label for="reminderbox" class="block text-gray-700 font-medium mb-2">Reminderbox :</label>
                                <div id="reminderbox" name="" class="block w-full p-2 border border-gray-300 rounded-md focus:ring focus:ring-blue-200"></div>
                            </div>
                            <!-- Hidden input to store the reminder content -->
                            <textarea name="reminderContent" id="hiddenReminderContent" style="display:none;"></textarea>
                    ...
                    <div class="bg-white shadow-md rounded-lg p-6 overflow-y-auto">
                        <h2 class="text-xl font-bold mb-2">Data Tagihan</h2>
                        <p class="mb-4 italic">Noted*: Silakan pilih metode pengiriman terlebih dahulu, kemudian pilih sekolah yang diinginkan. Setelah itu, lanjutkan dengan memilih siswa sebelum menentukan data tagihan yang akan ditampilkan.</p>
                        <form action="" method="get">
                            <div class="mb-4">
                                <div class="block w-full p-2 max-h-screen border border-gray-300 rounded-sm overflow-y-auto">
                                    <table id="tagihanTable" class="min-w-full bg-white">
                                        <thead>
                                            <tr class="bg-teal-800 text-white sticky top-0">
                                                ...
                                                <th class="py-2 px-4 border-b border-gray-300 text-left text-sm font-semibold">tanggal_tagihan</th>
                                                <th class="py-2 px-4 border-b border-gray-300 text-left text-sm font-semibold">tagihan</th>
                                                <th class="py-2 px-4 border-b border-gray-300 text-left text-sm font-semibold">tanggal_lunas</th>
    ...
    <script src="../js/s_scriptpilsis.js"></script>
    <!-- <script src="../js/scriptKelas.js"></script> -->
    <script src="../js/s_scriptReminder.js"></script>
    <script src="../js/s_scriptTagCheck.js"></script>
</body>
