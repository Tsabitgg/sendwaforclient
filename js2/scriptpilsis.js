...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const loadBillingData = () => {
        ...
        checkAllBtn.onclick = function(e) {
            ...
            document.querySelectorAll('.tagihan-checkbox').forEach(checkbox => {
                ...
                // Only check boxes for selected students
                if (selectedSiswaIds.includes(siswaId)) {
                    checkbox.checked = true;
                    tagihanCheckedStatus[checkbox.value] = true;
        ...
        uncheckAllBtn.onclick = function(e) {
            ...
            document.querySelectorAll('.tagihan-checkbox').forEach(checkbox => {
                ...
                delete tagihanCheckedStatus[checkbox.value];
                // Trigger change event to update reminder messages
                checkbox.dispatchEvent(new Event('change'));
            });
            updateReminderBox();
        ...
        fetch('../Config/fetchTagihan.php', {
            ...
            <td class="py-2 px-4 border-b border-gray-300">
                <input type="checkbox" value="${idTagihan}" class="tagihan-checkbox bg-transparent peer mr-2 appearance-none h-4 w-4 border-2 rounded-full hover:border-teal-500 cursor-pointer border-teal-300"
                    ...
                    data-nama-tagihan="${tagihan[columns.name_tagihan]}"
        ...
        `;
                        ...
                        tagihanDataMap[namaSiswa].push({
                            ...
                            name_tagihan: tagihan[columns.name_tagihan],
    ...
    //             const rincianTagihan = selectedTagihan.map(item => `\u00A0\u00A0\u00A0\u00A0*- ${item.name_tagihan || '-'}: Rp ${Number(item.tagihan).toLocaleString('id-ID')}*`).join('\n');
    ...
    //             // // Bersihkan pesan dari whitespace yang tidak diinginkan
    //             personalizedMessage = personalizedMessage
    //                 .split('\n')
    //                 .map(line => line.trim()) // Hapus whitespace di awal dan akhir setiap baris
    //                 .join('\n');
    ...
    //         // Menggunakan pre tag untuk mempertahankan formatting
    //         const formattedMessages = messages.map((msg, index) => {
    //             const messageHtml = `<div style="white-space: pre-line; text-align: justify;">${msg}</div>`;
                
    //             return index < messages.length - 1 
    //                 ? messageHtml + '<div style="border-top: 2px dashed #666; margin: 10px 0; color: #666;">'
    //                 : messageHtml;
    //         }).join('');
    
    //         reminderBox.innerHTML = formattedMessages;
    ...
    const updateReminderBox = () => {
        ...
        // Collect selected tagihan for each student
        Object.keys(tagihanCheckedStatus).forEach(tagihanId => {
            if (tagihanCheckedStatus[tagihanId]) {
                for (const siswa in tagihanDataMap) {
                    const tagihanList = tagihanDataMap[siswa];
        ...
        if (selectedTagihan.length === 0) {
            ...
            Object.keys(groupedTagihan).forEach(idSiswa => {
                ...
                // Filter tagihan details for the current student only
                const rincianTagihan = tagihanList.map(item => 
                    `\u00A0\u00A0\u00A0\u00A0*- ${item.name_tagihan || '-'}: Rp ${Number(item.tagihan).toLocaleString('id-ID')}*`
                ).join('\n');
            ...
            // Use <pre> to retain formatting and add dashed line between messages
            const formattedMessages = messages.map((msg, index) => {
                const messageHtml = `<div style="white-space: pre-line; text-align: justify;">${msg}</div>`;
                
                return index < messages.length - 1 
                    ? messageHtml + '<div style="border-top: 2px dashed #666; margin: 10px 0; color: #666;"></div>'
                    : messageHtml;
            }).join('');
